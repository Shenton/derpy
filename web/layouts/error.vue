<template>
    <b-container class="pt-5 pb-5 text-center align-middle">
        ...
        <h3>{{ message }}</h3>
        <p v-if="statusCode === 404"><NuxtLink to="/">{{ messages.back_to_home }}</NuxtLink></p>
        <p v-else-if="statusCode === 500">{{ messages.server_error }}</p>
        <p v-else-if="debug">{{ messages.client_error_details }}</p>
    </b-container>
</template>
